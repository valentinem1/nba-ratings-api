...
const fetchPlayers = async () => {
    try{
        ...
        //     // console.log(players[i].full_name);
        //     hashMap[players[i].full_name] = hashMap[players[i].full_name] + 1 || 1
        // };
    
        // for(let j = 0; j <= players.length-1; j++){
        //     if(hashMap[players[j].full_name] > 1){
        ...
        // players.forEach(dbPlayer => {
            ...
            // for(let year = 2010; year < 2020; year++){
                const workbook = XLSX.readFile(`${__dirname}/nba-stats-excel/nba-stats-2010.xlsx`);
                const sheet_name_list = workbook.SheetNames;
            
                const playersData = XLSX.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]])
                    for(let i = 0; i<= playersData.length-1; i++){
                        if(playersData[i]['Player'] !== playersData[i+1]['Player'] && playersData[i]['Player'] !== 'Player') {
                            // await Player.create({
                                // first_name: playersData[i]['Player'].split(' ')[0],
                            //     last_name: playersData[i]['Player'].split(' ')[1],
                            //     full_name: playersData[i]['Player']
                            // })
                        }
